...
An end-to-end automation pipeline that reads a [Trivy](https://github.com/aquasecurity/trivy) vulnerability report, uses Claude (claude-opus-4-6) to generate precise file patches, applies them to the repository, and opens a Pull Request — with zero human intervention in the fix cycle.
...
└── Auto-Remediation Stage  ← this tool
        ...
        ├── 3. Analyze      Claude reads vulns + your Dockerfile/manifests
...
```bash
git clone https://github.com/your-org/trivy-remediation-agent
cd trivy-remediation-agent
pip install -r requirements.txt
cp .env.example .env
# Edit .env — add ANTHROPIC_API_KEY and VCS_TOKEN
```
...
# 2. Run the agent (dry-run first to see what it would change)
python -m src.main \
...
Options:
  ...
  -n, --dry-run         No git ops, preview changes only
  --help
```
...
│   ├── llm_analyzer.py   Claude integration — returns a RemediationPlan
...
Claude returns a **structured JSON `RemediationPlan`** (validated with Pydantic) containing:
...
- The `ANTHROPIC_API_KEY` and `VCS_TOKEN` are read from environment variables and never written to disk.
